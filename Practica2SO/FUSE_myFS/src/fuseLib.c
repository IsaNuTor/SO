...
static int my_read(const char *path, char *buf, size_t size, off_t offset, struct fuse_file_info *fi)
{
	...
	bytes2Read = ((node->fileSize - offset) < size) ? (node->fileSize - offset) : size;
	...
	while (totalRead < bytes2Read)
	{
		...
		// Copy requested bytes without leaving the block
...
static int my_symlink(const char *path, const char *lpath)
{
	...
	if (strlen(lname) > myFileSystem.superBlock.maxLenFileName)
	{
		return -ENAMETOOLONG;
	}
	...
	if (findFileByName(&myFileSystem, lname) != -1)
	{
		return -EEXIST;
	}
	...
	myFileSystem.nodes[idxNodoI]->fileSize = size;
	myFileSystem.nodes[idxNodoI]->numBlocks = 0;
	myFileSystem.nodes[idxNodoI]->modificationTime = time(NULL);
	myFileSystem.nodes[idxNodoI]->freeNode = false;

	int numBlock = size / BLOCK_SIZE_BYTES;

	int numModBlock = size % BLOCK_SIZE_BYTES;

	if(numModBlock != 0) {
		++numBlock;
	}

	reserveBlocksForNodes(&myFileSystem, myFileSystem.nodes[idxNodoI]->blocks, numBlock);
	updateDirectory(&myFileSystem);
	updateNode(&myFileSystem, idxNodoI, myFileSystem.nodes[idxNodoI]);
	...
	/*COMPLETAR*/
	int write;
	if ((write = my_write(lpath, path, size+1, 0, &fi)) == -1)
	{
		return -ENOSYS;
	}
