...
int readInodes(MyFileSystem* myFileSystem)
{
    ...
    for (i=0;i<MAX_NODES;i++)
    {
		posInode = findNodeByPos(i);

		if (lseek(myFileSystem->fdVirtualDisk, posInode, SEEK_SET) == (off_t) - 1) {
		    perror("Failed lseek in readInodes()");
		    return -1;
		}
		if (read(myFileSystem->fdVirtualDisk, &Inode, sizeof (NodeStruct)) == -1) {
		...
		if (!Inode.freeNode)
		{
			...
			copyNode(myFileSystem->nodes[i],&Inode); 
		}else{
			myFileSystem->nodes[i]=NULL;
		}
    }
